...
function [q_array, fvals] = gif1_param_est(M)
%% Initialization
...
k_3_endo = 15000 + (25000 - 15000) * mymat(:, 1);
d_3_endo = 0.1 + (50 - 0.1) * mymat(:, 2);
d_5_cei = 0.01 + (10 - 0.01) * mymat(:, 3);
d_8_endo = 1 + (100 - 1) * mymat(:, 4);
d_1_vasc = 200 + (600 - 200) * mymat(:, 5);
d_4_endo = 1 + (200 - 1) * mymat(:, 6);
d_4_qc = 1 + (500 - 1) * mymat(:, 7);
k_4_vasc = 1000 + (20000 - 10000) * mymat(:, 8);
d_4_vasc = 1 + (100 - 1) * mymat(:, 9);
d_4_cei = 10 + (250 - 10) * mymat(:, 10);
d_8_cei = 0.01 + (1 - 0.01) * mymat(:, 11);

q_initial = [k_3_endo, d_3_endo, d_5_cei, d_8_endo, d_1_vasc, d_4_endo,...
    d_4_qc, k_4_vasc, d_4_vasc, d_4_cei, d_8_cei];
...
%% Residual calculation function
    function R = residual(q)
        ...
        R4 = (SCR(Tf==8)-143.88)^2 + (SCR(Tf==16)-154.58)^2 +...
            (SCR(Tf==24)-138.73)^2 + (SCR(Tf==32)-143.50)^2 +...
            (SCR(Tf==40)-150.26)^2 + (SCR(2880)-84.57)^2;

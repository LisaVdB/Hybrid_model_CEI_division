...
d_4_qc = 0.01 + (200 - 0.01) * rand(2 * M, 1);
...
d_4_cei = 0.01 + (1000 - 0.01) * rand(2 * M, 1);

% VASC
d_1_vasc = 0.01 + (1000 - 0.01) * rand(2 * M, 1);
...
d_4_endo = 0.01 + (200 - 0.01) * rand(2 * M, 1);

params = [k_1_qc,k_2_qc,k_3_qc,k_8_qc,K_D2_qc,K_D3_qc,K_D4_qc,d_3_qc,d_8_qc,d_4_qc,...
    k_2_cei,k_3_cei,k_5_cei,k_8_cei,K_D2_cei,K_D3_cei,K_D4_cei,d_3_cei,d_5_cei,...
    d_8_cei,d_4_cei,...
    d_1_vasc,k_4_vasc,K_D1_vasc,d_4_vasc,...
...
for i = 1:size(A, 2)
    ...
    for j = 1:M
        ...
        bparams = B(j, :);
        cparams = C(j, :);
        myfuna = @(t, x) gif1_dy_SA(t, x, aparams(1:num_params));
        myfunb = @(t, x) gif1_dy_SA(t, x, bparams(1:num_params));
        myfunc = @(t, x) gif1_dy_SA(t, x, cparams(1:num_params));
